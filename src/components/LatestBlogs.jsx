...
const LatestBlogs = () => {
  ...
  useEffect(() => {
    fetch("/api/blogsAPI")
      .then((res) => res.json())
      .then((blogs) => {
        setNewBlogs(blogs.slice(0, 4));
      })
      .catch((err) => console.log("problem fetching data"));
  }, []);
...
// <Link
//           href={`/blogs/${b._id}`}
//           className="h-full relative overflow-hidden group hover:scale-105 transition-all inline-block w-[250px] rounded-lg max-md:ml-[200px]"
//           key={i}
//         >
//           <div className="w-[220px] max-h-[270px] h-2/3">
//             <Image
...
//           <div className="bg-slate-900 flex flex-col gap-2 group-hover:bg-slate-800/50 px-2 py-1 transition-all ease-linear h-full w-full">
//             <h2 className="text-xl font-semibold">{b.blogTitle}</h2>
//             <p className="text-sm text-zinc-300 line-clamp-2 w-full">{b.blogIntro}
//                 Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ea nihil aperiam obcaecati cum! Aperiam eos deleniti mollitia dolore, debitis suscipit distinctio non deserunt natus esse minus repellendus iste a modi rerum voluptates laudantium ducimus consectetur ipsam asperiores. Quisquam reprehenderit sint quasi nisi doloremque quos corporis fuga, minima tempore voluptatum quas a perspiciatis consequuntur hic veritatis dolore illo explicabo ipsam nihil repudiandae incidunt! Libero numquam unde doloribus perspiciatis laboriosam quibusdam veniam, aperiam expedita necessitatibus eligendi repudiandae atque officiis. Molestiae eius illum quis veritatis doloremque provident, unde adipisci culpa recusandae eum ipsam explicabo vitae optio odit nihil possimus voluptas magnam! Eos, libero.
//             </p>
//           </div>
//         </Link>
